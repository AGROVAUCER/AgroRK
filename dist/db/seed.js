...
async function main() {
    ...
    // Insert data
    const cropRecords = await Promise.all(crops.map((c) => prisma_1.prisma.crop.upsert({
        where: { name_orgId: { name: c.name, orgId: org.id } },
        update: {},
        create: { name: c.name, orgId: org.id },
    })));
    const operationRecords = await Promise.all(operations.map((op) => prisma_1.prisma.operation.upsert({
        where: { name_orgId: { name: op.name, orgId: org.id } },
        update: {},
        create: { name: op.name, type: op.type, orgId: org.id },
    })));
    const fieldRecords = await Promise.all(fields.map((f) => {
        const cropId = cropRecords.find((c) => c.name === f.currentCrop)?.id;
        return prisma_1.prisma.field.upsert({
    ...
    const clientRecords = await Promise.all(clients.map((c) => prisma_1.prisma.client.upsert({
        where: { name_orgId: { name: c.name, orgId: org.id } },
        update: {},
        create: { name: c.name, phone: c.phone, location: c.location, orgId: org.id },
    })));
    const executorRecords = await Promise.all(executors.map((ex) => prisma_1.prisma.executor.upsert({
    ...
    await prisma_1.prisma.workEntry.createMany({
        data: [
            {
                date: today,
                type: "Rad",
    ...
    await prisma_1.prisma.yield.createMany({
        data: [
            {
                year: 2023,
                fieldId: fieldRecords[0].id,
